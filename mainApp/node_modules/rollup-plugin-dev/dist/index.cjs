...
const schema = Joi__default["default"].alternatives().try(
  ...
  Joi__default["default"].object({
    ...
    forcePort: Joi__default["default"].boolean(),
...
var dirs = fp__default["default"](async (server, { basePath, index, dirs, dirname = '' }) => {
  const prefix = basePath;
  const root = dirs.map(dir => path.resolve(dirname, dir));
  root.forEach(logDir(server));

  server.register(staticPlugin__default["default"], { prefix, root, index });
});
...
async function boot(config) {
  try {
    const resolvedPort = config.forcePort ? config.port : await getPort__default["default"]({ host: (config.host ?? '127.0.0.0'), port: [config.port, ...getPort.makeRange(8081, 9000)] });
