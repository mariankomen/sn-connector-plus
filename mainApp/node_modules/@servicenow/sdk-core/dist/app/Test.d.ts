...
import '@servicenow/sdk-core/runtime/global';
...
type FrequencyType = '' | 'weekly' | 'quarterly' | 'weekly2' | 'semiannual' | 'monthly' | 'yearly' | 'daily' | 'monthly2';
type UIActionAssertType = '' | 'form_submitted_to_server' | 'form_submission_canceled_in_browser' | 'page_reloaded_or_redirected';
...
type RestStatusOperationType = 'equals' | 'not_equals' | 'less_than' | 'greater_than' | 'less_than_equals' | 'greater_than_equals';
...
type AssertDeclarativeActionType = '' | 'form_submitted_to_server' | 'form_submission_canceled_in_browser' | 'page_reloaded_or_redirected';
...
export interface ATFType {
    ...
    catalog_SP: {
         ...
         */
        reviewOrderGuideSummary(inputs: {
            items: (string | Record<'sc_cat_item'>)[];
            price: string | number;
        }): void;
        /**
    ...
    server: {
         ...
         */
        addAttachmentsToExistingRecord(inputs: {
            recordId: string;
            table: string;
        }): void;
        /**
...
type FunctionWithId<T extends (param: any) => any> = (inputs: Now.Internal.WithID<Parameters<T>[0]> & StandardStepValues) => ReturnType<T>;
type CategoryWithId<T extends {
    [key: string | number | symbol]: (inputs: any) => any;
}> = {
    [key in keyof T]: FunctionWithId<T[key]>;
};
type ATFTypeWithID = {
    [key in keyof ATFType]: CategoryWithId<ATFType[key]>;
};
 ...
 */
export declare function Test(inputObject: InputObject, configurationFunction: (ATF: ATFTypeWithID) => any): {
    inputObject: InputObject;
    configurationFunction: (ATF: ATFTypeWithID) => any;
};
export {};
