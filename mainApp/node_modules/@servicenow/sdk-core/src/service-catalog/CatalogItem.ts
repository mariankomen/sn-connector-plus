...
type M2MRelationshipsBase = {
    ...
    variableSets?: { variableSet: string | VariableSet; order: number }[]
}
 ...
 */
type CatalogItemConfig<V extends VariableTypes = VariableTypes> = Now.Internal.WithID<
 ...
 */
type ExtractVariables<VS> = VS extends { variableSet: { variables?: infer Vars } } ? Vars : {}

/**
 * Recursively merges variables from an array of VariableSets
 */
type MergeVariableSets<VSets> = VSets extends readonly [infer First, ...infer Rest]
    ? ExtractVariables<First> & MergeVariableSets<Rest>
    : {}

/**
 * Combines direct variables with variables from variableSets
 */
type MergedVariables<V extends VariableTypes, VSets> = (V extends VariableTypes ? V : {}) &
    (VSets extends readonly any[] ? MergeVariableSets<VSets> : {})
...
export function CatalogItem<
    const V extends VariableTypes,
    const VSets extends NonNullable<M2MRelationships['variableSets']>,
>(
    config: CatalogItemConfig<V> & { variableSets?: VSets }
): CatalogItemConfig<V> & { variables: MergedVariables<V, VSets> }
// biome-ignore lint/suspicious/noRedeclare: function overloads require redeclaration
export function CatalogItem(config: CatalogItemConfig): CatalogItemConfig
export function CatalogItem(config: CatalogItemConfig) {
    return config
}
