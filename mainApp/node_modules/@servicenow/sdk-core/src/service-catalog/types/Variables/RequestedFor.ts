...
/**
 * Configuration type for requested for variables
 */
export type RequestedForVariableType = VariableConfig<Record<'sys_user'> | Now.Internal.ExplicitKey | string> & {
    /** Enable also request for */
    enableAlsoRequestFor?: boolean
    /** Roles to use also request for */
    rolesToUseAlsoRequestFor?: (string | Role)[]
} & {
...
/**
 * A Variable for a requested for field (typically references a user).
 *
 * @see https://docs.servicenow.com/csh?topicname=variable-types.html&version=latest
 *
 * @param config - Configuration for the requested for variable including:
 ...
 */
export function RequestedForVariable(config: RequestedForVariableType) {
    return config
}
