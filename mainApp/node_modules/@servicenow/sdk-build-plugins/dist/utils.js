...
function isRequire(callExpression) {
    ...
    return sdk_build_core_1.ts.Node.isIdentifier(expression) && expression.getText() === 'require';
}
/**
 ...
 */
function validateClientSideScript(script, compiler) {
    const source = compiler.createSourceFile('tmp-script.ts', script);
    const hasImports = source.getDescendantsOfKind(sdk_build_core_1.ts.SyntaxKind.ImportDeclaration).length > 0;
    const hasRequires = source.getDescendantsOfKind(sdk_build_core_1.ts.SyntaxKind.CallExpression).some(isRequire);
 ...
 */
function createSdkDocEntry(apiName, tags) {
    return {
        apiName,
