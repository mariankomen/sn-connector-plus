...
exports.SPWidgetDependencyPlugin = void 0;
const sdk_build_core_1 = require("@servicenow/sdk-build-core");
const utils_1 = require("../utils");
const now_id_plugin_1 = require("../now-id-plugin");
exports.SPWidgetDependencyPlugin = sdk_build_core_1.Plugin.create({
    ...
    shapes: [
        {
            ...
            async toRecord(callExpression, { diagnostics, factory }) {
                ...
                if ((!urlDefined && !uiScriptDefined) || (urlDefined && uiScriptDefined)) {
                    diagnostics.error(jsInclude, `JsInclude must define either a url or sysUiScript property, but not both`);
                }
                return {
                    success: true,
                    value: await factory.createRecord({
                        ...
                        properties: jsInclude.transform(({ $ }) => ({
                            ...
                            source: $.val(source),
                            sys_ui_script: $.from('sysUiScript').map(utils_1.toReference),
                            url: $.from('url').def(''),
                        })),
                    }),
                };
                ...
                return {
                    ...
                    value: await factory.createRecord({
                        ...
                        properties: cssInclude.transform(({ $ }) => ({
                            ...
                            source: $.val(source),
                            sp_css: $.from('spCss').map(utils_1.toReference),
                            url: $.from('url').def(''),
                        })),
                    }),
                };
        ...
        },
        {
            ...
            async toRecord(callExpression, { diagnostics, factory }) {
                ...
                const cssIncludes = await getIncludeRecords(dependency.get('cssIncludes'), dependencyRecord.getId(), factory, 'm2m_sp_dependency_css_include', 'sp_css_include', diagnostics);
                const jsIncludes = await getIncludeRecords(dependency.get('jsIncludes'), dependencyRecord.getId(), factory, 'm2m_sp_dependency_js_include', 'sp_js_include', diagnostics);
                return {
                    success: true,
...
async function getIncludeRecords(shape, dependencyId, factory, m2mTable, includeTable, diagnostics) {
    return (await Promise.all(shape?.ifArray()?.map(async (include) => {
        ...
        if (includeProps?.isString()) {
            ...
            diagnostics.error(includeProps, `include must contain a valid ${necessaryPlugin}, Record<'${includeTable}'> or sys_id'`);
            return undefined;
        }
        const m2mProps = {
...
//# sourceMappingURL=dependency-plugin.js.map