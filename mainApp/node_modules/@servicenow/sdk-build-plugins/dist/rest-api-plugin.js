...
function mergeAcls(acls) {
    if (acls?.isArray()) {
        return acls?.pipe((r) => Array.from(new Set(r.getElements().map((r) => r.ifRecord()?.getId().getValue() ?? r.getValue()))).join(','));
    }
...
function getVersionNumber(versionShape, versions) {
...
function getAttributeRecord(attrAssociationRecord, attrRecords, attrType) {
...
function getVersionId(versionShape, versionToVersionRecordMap, diagnostics) {
    const version = versionShape.ifNumber()?.getValue() ?? undefined;
    if (!version) {
        return '';
    }
    if (!versionToVersionRecordMap.has(`v${version}`)) {
        diagnostics.error(versionShape, `Unable to resolve version details for version number "${version}". Check if the version is listed in your API versions list.`);
        return '';
    }
    return versionToVersionRecordMap.get(`v${version}`).getId();
}
async function generateVersionRecords(versions, restApi, restRecord, defaultVersion, factory, diagnostics, callExpression) {
    ...
    for (const v of versions) {
        ...
        if (versionMap.has(versionId)) {
            diagnostics.error(v.asObject().get('version'), `Duplicate versions with version number "${version}" found in RestApi with id "${restApi.get('$id').getValue()}". All versions are expected to be unique.`);
        }
        versionMap.set(versionId, versionRecord);
    }
    return versionMap;
}
function checkForDuplicateRecords(recordMap, record, attrShape, diagnostics) {
    ...
    if (!recordMap.has(recordId)) {
        ...
        const originalShape = sdk_build_core_1.Shape.from(record.getSource(), record.getValue());
        const matchedShape = sdk_build_core_1.Shape.from(matchRec.getSource(), matchRec.getValue());
        if (!matchedShape.equals(originalShape)) {
            diagnostics.error(attrShape.get('$id'), `Multiple route attributes found with the same ID "${attrShape.get('$id').getValue()}" but with different property values. Attributes with same ID are expected to have identical property values.`);
        }
    }
}
async function generateRouteAttributeRecords(attributes, restDef, routeDef, attrType, attrMap, factory, diagnostics) {
    const records = [];
...
async function generateRouteAndRouteAttrRecords(routes, restDef, factory, config, diagnostics, versionToVersionRecordMap, callExpression) {
    const routeRecords = [];
    const headersMap = new Map();
    const parametersMap = new Map();
    for (const r of routes) {
        const route = r.asObject();
        const routeVersion = route.get('version').getValue();
        ...
        if (route.get('script').isUnresolved()) {
            diagnostics.error(route.get('script').getOriginalNode(), `Unable to resolve the script reference, ensure the imported module is within the ${config.serverModulesDir} directory.`);
        }
        const routeRecord = await factory.createRecord({
            ...
            properties: route.transform(({ $ }) => ({
                ...
                operation_script: $.from('script')
                    .map((v) => v.if(server_module_plugin_1.ModuleFunctionShape)?.toString((n) => `${n}({{PARAMS}})`, ['request', 'response']) ?? v)
                    .toCdata()
                    .def(''),
        ...
        routeHeaders.forEach((h) => {
            ...
            (0, utils_1.generateDeprecatedDiagnostics)(h.asObject(), diagnostics);
        });
        const headerRecords = await generateRouteAttributeRecords(routeHeaders, restDef, routeRecord, 'header', headersMap, factory, diagnostics);
        ...
        routeParameters.forEach((p) => {
            ...
            (0, utils_1.generateDeprecatedDiagnostics)(p.asObject(), diagnostics);
        });
        const parameterRecords = await generateRouteAttributeRecords(routeParameters, restDef, routeRecord, 'query_parameter', parametersMap, factory, diagnostics);
        routeRecords.push(routeRecord.with(...headerRecords, ...parameterRecords));
    }
    return routeRecords;
}
function findOrphanedAttributes(attributes, attributeRouteAssociations, attributeType) {
    const attributeIdToRecordMap = new Map();
    attributes.forEach((attribute) => {
        attributeIdToRecordMap.set(attribute.getId().getValue(), attribute);
    });
    const attributeIds = new Set(attributeIdToRecordMap.keys());
    const referencedAttributeIds = new Set(attributeRouteAssociations.map((m2m) => {
...
function validateAttributesAndAssociations(attributes, attributeRouteAssociations, attributeType) {
    const attributeIdToRecordMap = new Map();
    attributes.forEach((attribute) => {
        attributeIdToRecordMap.set(attribute.getId().getValue(), attribute);
    });
    ...
    if (invalidAttributeAssociations.length > 0) {
        const invalidAttributeAssociationIds = invalidAttributeAssociations
            ...
            .join(', ');
        throw new Error(`Found ${invalidAttributeAssociations.length} ${attributeType} association record(s) referencing non-existent ${attributeType}s: ${invalidAttributeAssociationIds}. ` +
            `Please clean up these m2m records.`);
    }
}
function routeAttributeTransform(associationRecords, attributes, operationFilterId, attributeType) {
    const results = associationRecords
        ...
        return attribute
            .transform(({ $ }) => ({
            $id: $.val(now_id_plugin_1.NowIdShape.from(attribute)),
            name: $,
...
function versionsTransform(versions) {
    if (versions.length === 0) {
        return undefined;
    }
    return versions.map((v) => v
        .transform(({ $ }) => ({
        $id: $.val(now_id_plugin_1.NowIdShape.from(v)),
...
exports.RestApiPlugin = sdk_build_core_1.Plugin.create({
    name: 'RestApiPlugin',
    records: {
        sys_ws_header_map: {
        ...
        sys_ws_definition: {
            ...
            async toShape(record, { descendants, transform }) {
                ...
                const routesWithScript = await Promise.all(routes.map(async (r) => {
                    const script = await now_include_plugin_1.NowIncludeShape.fromRecord(r, r.get('operation_script'), transform);
                    return r
                        .transform(({ $ }) => ({
                        $id: $.val(now_id_plugin_1.NowIdShape.from(r)),
                        name: $,
                        active: $.toBoolean().def(true),
                        consumes: $,
                        ...
                        parameters: $.val(routeAttributeTransform(paramRouteAssociations, parameters, r.getId().getValue(), 'query_parameter')).def([]),
                        headers: $.val(routeAttributeTransform(headerRouteAssociations, headers, r.getId().getValue(), 'header')).def([]),
                        version: $.from('web_service_version').map((v) => getVersionNumber(v, versions)),
                    }))
                        .withAliasedKeys(routeAliases);
                }));
                const shape = new sdk_build_core_1.CallExpressionShape({
                    source: record,
                    callee: 'RestApi',
                    args: [
                        record
    ...
    shapes: [
        {
            ...
            async toRecord(callExpression, { factory, config, diagnostics }) {
                ...
                (0, utils_1.generateDeprecatedDiagnostics)(restApi, diagnostics);
                ...
                if (!serviceId.asString().getValue().match(serviceIdRegex)) {
                    diagnostics.error(serviceId, `Rest service_id must only contain lowercase letters, numbers, and underscores and end with a letter or number`);
                }
                const versions = restApi.get('versions')?.ifArray()?.getElements() ?? [];
                versions.forEach((v) => {
                    v.asObject().withAliasedKeys(versionAliases);
                    (0, utils_1.generateDeprecatedDiagnostics)(v.asObject(), diagnostics);
                });
                const defaultVersions = versions.filter((v) => v.asObject().get('isDefault').ifBoolean()?.getValue() ?? false);
                const defaultVersion = defaultVersions.length === 1 ? defaultVersions[0].asObject().get('version').getValue() : -1;
                ...
                const restRecord = await factory.createRecord({
                    ...
                    properties: restApi.transform(({ $ }) => ({
                        ...
                        short_description: $.from('shortDescription').def(''),
                        consumes: $.def(DEFAULT_MEDIA_TYPE),
                        consumes_customized: $.from('consumes').map((v) => v.isString() ? v.getValue() !== DEFAULT_MEDIA_TYPE : false),
                        default_version: $.val(defaultVersion !== -1 ? `v${defaultVersion}` : 'No active default version'),
                        is_versioned: $.val(versions.length > 0 ? true : false),
                        namespace: $.val(scope),
                        doc_link: $.from('docLink').def(''),
                        produces: $.def(DEFAULT_MEDIA_TYPE),
                        produces_customized: $.from('produces').map((v) => v.isString() ? v.getValue() !== DEFAULT_MEDIA_TYPE : false),
                        sys_policy: $.from('policy').def(''),
                        enforce_acl: $.val(mergeAcls(enforceAcls)),
                    })),
                });
                const versionToVersionRecordMap = await generateVersionRecords(versions, restApi, restRecord, defaultVersion, factory, diagnostics, callExpression);
                const routes = restApi.get('routes')?.ifArray()?.getElements() ?? [];
                routes.forEach((r) => {
                    r.asObject().withAliasedKeys(routeAliases);
                    (0, utils_1.generateDeprecatedDiagnostics)(r.asObject(), diagnostics);
                });
                const routeAndRouteAttrRecords = await generateRouteAndRouteAttrRecords(routes, restRecord, factory, config, diagnostics, versionToVersionRecordMap, callExpression);
                return {
                    success: true,
...
//# sourceMappingURL=rest-api-plugin.js.map