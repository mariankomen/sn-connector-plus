...
exports.SLAPlugin = void 0;
const sdk_build_core_1 = require("@servicenow/sdk-build-core");
const now_id_plugin_1 = require("./now-id-plugin");
const sla_validators_1 = require("./sla/sla-validators");
 ...
 */
exports.SLAPlugin = sdk_build_core_1.Plugin.create({
    name: 'SLAPlugin',
    records: {
        contract_sla: {
                        ...
                        args: [
                            record.transform(({ $ }) => ({
                                ...
                                duration: $.from('duration').map((v) => {
                                    ...
                                    if (!stringShape) {
                                        return sdk_build_core_1.DurationShape.zero(record);
                                    }
                                    const value = stringShape.getValue();
                                    if (value === DEFAULT_DURATION_ON_PLATFORM) {
                                        return sdk_build_core_1.DurationShape.zero(record);
                                    }
                                    return sdk_build_core_1.DurationShape.from(record, stringShape);
                                }),
                                ...
                                relativeDurationWorksOn: $.from('relative_duration_works_on').def(DEFAULT_RELATIVE_DURATION_WORKS_ON),
                                // Conditions
                                conditions: $.from('start_condition', 'stop_condition', 'pause_condition', 'resume_condition', 'reset_condition', 'cancel_condition').map((start_condition, stop_condition, pause_condition, resume_condition, reset_condition, cancel_condition) => {
                                    const conditions = {};
                                ...
                                whenTo: $.from('when_to_resume', 'when_to_cancel').map((when_to_resume, when_to_cancel) => {
                                    const resumeValue = when_to_resume.ifString()?.getValue();
                                    const cancelValue = when_to_cancel.ifString()?.getValue();
                                    return {
                                        resume: resumeValue || sla_validators_1.DEFAULT_WHEN_TO_RESUME,
                                        cancel: cancelValue || sla_validators_1.DEFAULT_WHEN_TO_CANCEL,
                                    };
                                }),
                                // Retroactive
                                retroactive: $.from('retroactive', 'retroactive_pause', 'set_start_to').map((retroactive, retroactive_pause, set_start_to) => {
                                    ...
                                    if (startValue) {
                                        result.pause = pauseValue ?? sla_validators_1.DEFAULT_RETROACTIVE_PAUSE;
                                    }
                                ...
                                // Time Zones
                                timezone: $.def(''),
                                timezoneSource: $.from('timezone_source').def(sla_validators_1.DEFAULT_TIMEZONE_SOURCE),
                                // Metadata
    ...
    shapes: [
        {
            ...
            async toRecord(callExpression, { factory, diagnostics }) {
                ...
                const whenToResumeValue = whenTo?.get('resume')?.ifString()?.getValue() || sla_validators_1.DEFAULT_WHEN_TO_RESUME;
                const whenToCancelValue = whenTo?.get('cancel')?.ifString()?.getValue() || sla_validators_1.DEFAULT_WHEN_TO_CANCEL;
                const timezoneSource = config.get('timezoneSource');
                const timezoneSourceValue = timezoneSource?.ifString()?.getValue() || sla_validators_1.DEFAULT_TIMEZONE_SOURCE;
                ...
                if (isUserSpecifiedOrEmptyDuration) {
                    ...
                    if (duration && duration.isDefined()) {
                        if (!(duration instanceof sdk_build_core_1.CallExpressionShape) || duration.getCallee() !== 'Duration') {
                            diagnostics.error(duration, "SLA 'duration' field must use the Duration() helper function");
                        }
                    }
                    (0, sla_validators_1.validateUserSpecifiedDuration)(config, diagnostics);
                }
                else {
                    // Behavior 2: Relative duration
                    (0, sla_validators_1.validateRelativeDuration)(config, conditions, diagnostics);
                }
                // Schedule Source Validations
                if (scheduleSourceValue === 'no_schedule') {
                    // Behavior 5: No schedule
                    (0, sla_validators_1.validateNoScheduleRestrictions)(config, diagnostics);
                }
                else if (scheduleSourceValue === 'task_field') {
                    // Behavior 6: Task field schedule
                    (0, sla_validators_1.validateTaskFieldSchedule)(config, diagnostics);
                }
                else if (scheduleSourceValue === 'sla_definition') {
                    // Behavior 7: SLA definition schedule (default)
                    (0, sla_validators_1.validateSlaDefinitionSchedule)(config, diagnostics);
                }
                // Condition Field Validations
                // Behavior 8: Timezone source
                (0, sla_validators_1.validateTimezoneSource)(config, timezoneSourceValue, diagnostics);
                // Behavior 9: When to resume = "no_match"
                if (whenToResumeValue === 'no_match') {
                    (0, sla_validators_1.validateNoMatchResume)(conditions, diagnostics);
                }
                // Behavior 10: When to cancel validation
                (0, sla_validators_1.validateCancelCondition)(whenToCancelValue, conditions, diagnostics);
                // Retroactive Validations
                if (retroactiveStartValue && retroactive) {
                    // Behaviors 6 & 8: Retroactive start validations
                    (0, sla_validators_1.validateRetroactiveStart)(retroactive, isUserSpecifiedOrEmptyDuration, diagnostics);
                }
                else {
                    // Behavior 7: Validate that retroactive-dependent fields are not set when retroactive.start is false
                    (0, sla_validators_1.validateRetroactiveFieldsWithoutStart)(retroactive, diagnostics);
                }
                return {
                    success: true,
                    value: await factory.createRecord({
                        ...
                        properties: config.transform(({ $ }) => ({
                            ...
                            schedule: $.map((v) => {
                                if (scheduleSourceValue === 'no_schedule') {
                                    return sla_validators_1.DEFAULT_NO_SCHEDULE_ID;
                                }
                            ...
                            relative_duration_works_on: $.from('relativeDurationWorksOn').def(DEFAULT_RELATIVE_DURATION_WORKS_ON),
                            // Conditions
                            ...
                            when_to_resume: $.val(whenTo?.get('resume')).def(sla_validators_1.DEFAULT_WHEN_TO_RESUME),
                            when_to_cancel: $.val(whenTo?.get('cancel')).def(sla_validators_1.DEFAULT_WHEN_TO_CANCEL),
                            // Retroactive
                            ...
                            retroactive_pause: $.map(() => {
                                ...
                                return isUserSpecifiedOrEmptyDuration ? sla_validators_1.DEFAULT_RETROACTIVE_PAUSE : undefined;
                            }),
                            ...
                            // Time Zones
                            timezone: $,
                            timezone_source: $.from('timezoneSource').def(sla_validators_1.DEFAULT_TIMEZONE_SOURCE),
                            // Metadata
...
//# sourceMappingURL=sla-plugin.js.map