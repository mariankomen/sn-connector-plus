 ...
 */
const isValidActionValue = (value) => {
...
const isValidScript = (scriptValue) => scriptValue?.isString() && scriptValue.getValue() !== '' && scriptValue.getValue() !== DEFAULT_SCRIPT;
// Helper function to validate a script property for import/require statements
const validateScriptProperty = (scriptProperty, compiler, diagnostics) => {
    if (scriptProperty?.isString()) {
        const scriptContent = scriptProperty.getValue();
        if (scriptContent && !(0, utils_1.validateClientSideScript)(scriptContent, compiler)) {
            diagnostics.error(scriptProperty, 'UI Policy scripts cannot import or require modules. Scripts run client-side in the browser.');
        }
    }
};
exports.UiPolicyPlugin = sdk_build_core_1.Plugin.create({
    name: 'UiPolicyPlugin',
    records: {
        [SYS_UI_POLICY]: {
                        ...
                        args: [
                            record.transform(({ $ }) => {
                                ...
                                return {
                                    ...
                                    scriptTrue: $.val(includeScriptTrue ? (scriptTrueValue?.getValue() ?? DEFAULT_SCRIPT) : undefined),
                                    scriptFalse: $.val(includeScriptFalse
                                        ? (scriptFalseValue?.getValue() ?? DEFAULT_SCRIPT)
                                        : undefined),
                                    // uiType is REQUIRED when runScripts is true, should be omitted when false
                                    uiType: $.val(runScriptsValue
                                    ...
                                    relatedListActions: $.val(relatedListActions.length > 0 ? relatedListActions : undefined),
                                };
                            }),
                        ],
                    }),
    ...
    shapes: [
        {
            ...
            async toRecord(callExpression, { config, factory, diagnostics, compiler }) {
                ...
                if (tableValue?.isString() && config?.scope) {
                    ...
                    if (!tableName.startsWith(`${scopeName}_`) && !(0, sdk_build_core_1.isSNScope)(scopeName) && scopeName !== 'global') {
                        diagnostics.error(arg.get('table'), `'table' property should start with scope prefix '${scopeName}_'. UI Policies in scoped apps can only be created for tables within the same scope.`);
                    }
                }
                ...
                if (runScripts.isUndefined() || runScripts.ifBoolean()?.equals(false)) {
                    ...
                    [scriptTrue, scriptFalse]
                        ...
                        .forEach((shape) => diagnostics.info(shape, `This script will not execute unless runScripts is set to true`));
                }
                ...
                const uiTypeId = uiTypeValue?.isString()
                    ? getUITypeId(uiTypeValue.getValue())
                ...
                if (actions?.isArray()) {
                    ...
                    for (let i = 0; i < elements.length; i++) {
                        ...
                        if (!hasVisibleProp && !hasReadOnlyProp && !hasMandatoryProp && !hasClearedProp) {
                            diagnostics.error(action, `Action at index ${i} must specify at least one of: visible, readOnly, mandatory, or cleared`);
                            continue;
                        }
                ...
                const relatedListActionRecords = [];
                    ...
                    for (let i = 0; i < rlElements.length; i++) {
                        ...
                        if (list) {
                            if (list.isRecord()) {
                                ...
                                if ((0, sdk_build_core_1.isGUID)(listStr)) {
                                    listValue = `REL:${listStr}`;
                                }
                                else if (listStr.includes('.')) {
                                    const [table, field] = listStr.split('.');
                                    if (!table || !field || listStr.split('.').length !== 2) {
                                        diagnostics.error(list, `Related list action at index ${i}: 'list' property must be in 'table.field' format (e.g., 'incident.caller_id')`);
                                        continue;
                                    }
                                    listValue = listStr;
                                }
                                else {
                                    diagnostics.error(list, `Related list action at index ${i}: 'list' property must be either a Record<'sys_relationship'>, a GUID, or in 'table.field' format (e.g., 'incident.caller_id')`);
                                    continue;
                                }
                            }
                        }
                        ...
                        if (!hasVisibleProp && !listValue) {
                            diagnostics.error(rlAction, `Related list action at index ${i} must specify at least one of: list or visible`);
                            continue;
                        }
...
//# sourceMappingURL=ui-policy-plugin.js.map