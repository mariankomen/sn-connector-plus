...
exports.EmailNotificationPlugin = void 0;
...
const getNestedObjectProp = (arg, key) => {
...
const showGuidFieldDiagnostic = (inputReceived, fieldName, tableName, diagnostics) => {
    if (inputReceived) {
        const receivedValue = inputReceived.getValue();
        diagnostics.error(inputReceived, `'${fieldName}' must be a valid GUID or a Record<'${tableName}'>. Received: '${receivedValue}'`);
    }
};
exports.EmailNotificationPlugin = sdk_build_core_1.Plugin.create({
                    ...
                    value: new sdk_build_core_1.CallExpressionShape({
                        ...
                        args: [
                            record.transform(({ $ }) => ({
                                ...
                                triggerConditions: $.from('generation_type', 'action_insert', 'action_update', 'event_name', 'weight', 'order', 'condition', 'advanced_condition', 'item_table', 'affected_field_on_event', 'item').map((genType, actionInsert, actionUpdate, eventName, weight, order, condition, advancedCondition, itemTable, affectedField, item) => {
                                    const triggerConditions = {
                                        generationType: (genType?.getValue() ?? 'engine'),
                                    };
                                    const genTypeValue = genType?.getValue() ?? 'engine';
                                    if (genTypeValue === 'engine') {
                                        addProperty(triggerConditions, 'onRecordInsert', actionInsert?.toBoolean().getValue());
                                        addProperty(triggerConditions, 'onRecordUpdate', actionUpdate?.toBoolean().getValue());
                                    }
                                    ...
                                    addProperty(triggerConditions, 'advancedCondition', advancedCondition?.getValue());
                                    addProperty(triggerConditions, 'itemTable', itemTable?.getValue());
                                    addProperty(triggerConditions, 'affectedFieldOnEvent', affectedField?.getValue());
                                    addProperty(triggerConditions, 'item', item?.getValue());
                                    return triggerConditions;
                                }),
                                // Build recipientDetails using $.from().map()
                                recipientDetails: $.from('recipient_users', 'recipient_fields', 'recipient_groups', 'exclude_delegates', 'subscribable', 'send_self', 'event_parm_1', 'event_parm_2').map((users, fields, groups, excludeDelegates, subscribable, sendSelf, eventParm1, eventParm2) => {
                                    const recipientDetails = {};
                                    addProperty(recipientDetails, 'recipientUsers', commaStringToArray(users?.getValue()));
                                    addProperty(recipientDetails, 'recipientFields', commaStringToArray(fields?.getValue()));
                                    addProperty(recipientDetails, 'recipientGroups', commaStringToArray(groups?.getValue()));
                                    addProperty(recipientDetails, 'excludeDelegates', excludeDelegates?.toBoolean().getValue());
                                    addProperty(recipientDetails, 'isSubscribableByAllUsers', subscribable?.toBoolean().getValue());
                                    addProperty(recipientDetails, 'sendToCreator', sendSelf?.toBoolean().getValue());
                                    if (generationType === 'event') {
                                        addProperty(recipientDetails, 'eventParm1WithRecipient', eventParm1?.toBoolean().getValue());
                                        addProperty(recipientDetails, 'eventParm2WithRecipient', eventParm2?.toBoolean().getValue());
                                    }
                                    return Object.keys(recipientDetails).length > 0 ? recipientDetails : undefined;
                                }),
                                // Build emailContent using $.from().map()
                                emailContent: $.from('content_type', 'message_html', 'message_text', 'template', 'style', 'subject', 'message', 'sms_alternate', 'importance', 'from', 'reply_to', 'message_list', 'include_attachments', 'omit_watermark', 'push_message_only', 'force_delivery').map((contentTypeField, messageHtml, messageText, template, style, subject, message, smsAlternate, importance, from, replyTo, messageList, includeAttachments, omitWatermark, pushMessageOnly, forceDelivery) => {
                                    const emailContent = {};
                                    const contentTypeValue = contentTypeField?.getValue() || 'text/html';
                                    if (contentTypeValue !== 'text/html') {
                                        emailContent.contentType = contentTypeValue;
                                    }
                                    if (contentTypeValue === 'text/html' ||
                                    ...
                                    addProperty(emailContent, 'pushMessageList', commaStringToArray(messageList?.getValue()));
                                    addProperty(emailContent, 'includeAttachments', includeAttachments?.toBoolean().getValue());
                                    addProperty(emailContent, 'omitWatermark', omitWatermark?.toBoolean().getValue());
                                    addProperty(emailContent, 'pushMessageOnly', pushMessageOnly?.toBoolean().getValue());
                                    addProperty(emailContent, 'forceDelivery', forceDelivery?.toBoolean().getValue());
                                    return Object.keys(emailContent).length > 0 ? emailContent : undefined;
                                }),
                                // Build digest using $.from().map()
                                digest: $.from('digestable', 'default_digest', 'default_interval', 'digest_type', 'digest_template', 'digest_subject', 'digest_html', 'digest_text', 'digest_separator_html', 'digest_separator_text', 'digest_from', 'digest_reply_to').map((digestable, defaultDigest, defaultInterval, digestType, digestTemplate, digestSubject, digestHtml, digestText, separatorHtml, separatorText, digestFrom, digestReplyTo) => {
                                    const allowDigest = digestable?.toBoolean().getValue();
                                    if (!allowDigest) {
                                        return undefined;
                                    }
                                    const digest = { allow: true };
                                    ...
                                    const separatorHtmlValue = separatorHtml?.getValue();
                                    if (separatorHtmlValue &&
                                        separatorHtmlValue !== DEFAULT_DIGEST_SEPARATOR_HTML) {
                                        digest.separatorHtml = separatorHtmlValue;
                                    }
                                    const separatorTextValue = separatorText?.getValue();
                                    if (separatorTextValue &&
    ...
    shapes: [
        {
            ...
            async toRecord(callExpression, { factory, diagnostics }) {
                    ...
                    if (recipientUsersArg?.isArray()) {
                        ...
                        elements.forEach((element, index) => {
                            if (element.isRecord()) {
                                ...
                                if ((0, sdk_build_core_1.isGUID)(str) || isValidEmail(str)) {
                                    userList.push(str);
                                }
                                else {
                                    diagnostics.error(element, `'recipientDetails.recipientUsers[${index}]' must be a valid GUID or email address or a Record<'sys_user'>. Received: '${str}'`);
                                }
                            }
                        });
                    ...
                    if (recipientGroupsArg?.isArray()) {
                        ...
                        elements.forEach((element, index) => {
                            if (element.isRecord()) {
                                ...
                                if ((0, sdk_build_core_1.isGUID)(str)) {
                                    guidList.push(str);
                                }
                                else {
                                    showGuidFieldDiagnostic(element, `recipientDetails.recipientGroups[${index}]`, 'sys_user_group', diagnostics);
                                }
                            }
                        });
                        ...
                        elements.forEach((element, index) => {
                            if (element.isRecord()) {
                                ...
                                if ((0, sdk_build_core_1.isGUID)(str)) {
                                    ...
                                    showGuidFieldDiagnostic(element, `emailContent.pushMessageList[${index}]`, 'sys_push_notif_msg', diagnostics);
                                }
                            }
                        });
                ...
                const digestSeparatorHtml = digest?.get('separatorHtml')?.isDefined()
                    ...
                    : sdk_build_core_1.Shape.from(callExpression, DEFAULT_DIGEST_SEPARATOR_HTML).asString().withContentType('cdata');
                const digestSeparatorText = digest?.get('separatorText')?.isDefined()
                    ? digest.get('separatorText').asString().withContentType('cdata')
                    : sdk_build_core_1.Shape.from(callExpression, DEFAULT_DIGEST_SEPARATOR_TEXT).asString().withContentType('cdata');
                ...
                if (emailContent) {
                    ...
                    if (templateArg) {
                        if (templateArg.isRecord()) {
                            ...
                            if ((0, sdk_build_core_1.isGUID)(templateStr)) {
                                templateValue = templateStr;
                            }
                            else {
                                showGuidFieldDiagnostic(templateArg, 'emailContent.template', 'sysevent_email_template', diagnostics);
                            }
                        }
                    }
                }
                ...
                if (emailContent) {
                    ...
                    if (styleArg) {
                        if (styleArg.isRecord()) {
                            ...
                            // If it's a string GUID, use it
                            if ((0, sdk_build_core_1.isGUID)(styleStr)) {
                                styleValue = styleStr;
                            }
                            else {
                ...
                if (digest) {
                    ...
                    if (defaultIntervalArg) {
                        if (defaultIntervalArg.isRecord()) {
                            ...
                            // If it's a string GUID, use it
                            if ((0, sdk_build_core_1.isGUID)(defaultIntervalStr)) {
                                defaultIntervalValue = defaultIntervalStr;
                            }
                            else {
                                showGuidFieldDiagnostic(defaultIntervalArg, 'digest.defaultInterval', 'sys_email_digest_interval', diagnostics);
                            }
                        }
                    }
                }
                ...
                if (digest) {
                    ...
                    if (digestTemplateArg) {
                        if (digestTemplateArg.isRecord()) {
                            ...
                            // If it's a string GUID, use it
                            if ((0, sdk_build_core_1.isGUID)(digestTemplateStr)) {
                                digestTemplateValue = digestTemplateStr;
                            }
                            else {
                                showGuidFieldDiagnostic(digestTemplateArg, 'digest.template', 'sysevent_email_template', diagnostics);
                            }
                        }
                    }
                }
                ...
                if (generationType === 'engine' && triggerConditions) {
                    ...
                    if (!onRecordInsert && !onRecordUpdate) {
                        diagnostics.error(triggerConditions, "When triggerConditions.generationType is 'engine', at least one of 'onRecordInsert' or 'onRecordUpdate' must be true.");
                    }
                }
                ...
                if (generationType === 'event' && triggerConditions) {
                    ...
                    if (!eventName || (typeof eventName === 'string' && eventName.trim() === '')) {
                        diagnostics.error(triggerConditions, "When triggerConditions.generationType is 'event', 'eventName' is required.");
                    }
                }
                ...
                if (notificationType === 'vcalendar' && digest) {
                    ...
                    if (allowDigest === true) {
                        diagnostics.warn(digest, "Digest functionality is not supported for notificationType 'vcalendar'. The digest configuration will be ignored.");
                    }
                }
                ...
                if (generationType !== 'event' && recipientDetails) {
                    ...
                    if (eventParm1 === true || eventParm2 === true) {
                        diagnostics.warn(recipientDetails, `'eventParm1WithRecipient' and 'eventParm2WithRecipient' only apply when triggerConditions.generationType is 'event'. Current type is '${generationType}'.`);
                    }
                }
                ...
                if (emailContent && !emailContent.get('template')?.getValue()) {
                    ...
                    if (contentType === 'text/html') {
                        ...
                        if (!hasMessageHtml || !messageHtmlValue) {
                            diagnostics.warn(emailContent, `Content type is 'text/html' but 'messageHtml' is not provided. Please provide 'messageHtml' for HTML content.`);
                        }
                        // Warn if messageText is provided (it will be ignored)
                        if (hasMessageText && messageTextValue) {
                            diagnostics.warn(emailContent, `Content type is 'text/html': 'messageText' will be ignored. Only 'messageHtml' will be used.`);
                        }
                    }
                    else if (contentType === 'text/plain') {
                        // Warn if messageText is missing
                        if (!hasMessageText || !messageTextValue) {
                            diagnostics.warn(emailContent, `Content type is 'text/plain' but 'messageText' is not provided. Please provide 'messageText' for plain text content.`);
                        }
                        // Warn if messageHtml is provided (it will be ignored)
                        if (hasMessageHtml && messageHtmlValue) {
                            diagnostics.warn(emailContent, `Content type is 'text/plain': 'messageHtml' will be ignored. Only 'messageText' will be used.`);
                        }
                    }
                    else if (contentType === 'multipart/mixed') {
                        // Warn if both are missing
                        const hasAnyContent = (hasMessageHtml && messageHtmlValue) || (hasMessageText && messageTextValue);
                        if (!hasAnyContent) {
                            diagnostics.warn(emailContent, `Content type is 'multipart/mixed' but neither 'messageHtml' nor 'messageText' is provided. Please provide at least one or both.`);
                        }
                    }
                }
                ...
                if (categoryArg) {
                    if (categoryArg.isRecord()) {
                        ...
                        // If it's a string GUID, use it
                        if ((0, sdk_build_core_1.isGUID)(categoryStr)) {
...
//# sourceMappingURL=email-notification-plugin.js.map