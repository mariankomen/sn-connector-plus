            ...
            async toRecord(callExpression, { diagnostics, factory }) {
                ...
                const uiAction = await factory.createRecord({
                    ...
                    properties: arg.transform(({ $ }) => ({
                        ...
                        messages: $.val(arg.get('messages')?.isArray()
                            ? arg.get('messages').asArray().getValue().join('\n')
                            : arg.get('messages')).def([]),
                        list_style: $.val(arg.get(['list', 'style'])),
                        form_style: $.val(arg.get(['form', 'style'])),
                        condition: $.def(''),
                        script: $.map((v) => v.if(server_module_plugin_1.ModuleFunctionShape)?.toString((n) => `${n}({{PARAMS}})`, ['current']) ?? v)
                            .def('')
                            .toCdata(),
                        hint: $.def(''),
                ...
                for (const role of roles) {
                    const roleReference = role.isString()
                        ...
                        : role;
                    roleRecords.push(await factory.createRecord({
                        source: callExpression,
...
//# sourceMappingURL=ui-action-plugin.js.map