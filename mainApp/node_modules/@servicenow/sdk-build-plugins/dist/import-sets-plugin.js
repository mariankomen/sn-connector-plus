...
exports.ImportSetPlugin = void 0;
 ...
 */
const DEFAULT_TRANSFORM_MAP_SCRIPT = '(function transformRow(source, target, map, log, isUpdate) {\n\n\t// Add your code here\n\n})(source, target, map, log, action==="update");';
/**
 ...
 */
function hasAdditionalProperties(config) {
    return !!(config['coalesce']?.asBoolean()?.getValue() ||
 ...
 */
function buildFieldConfig(sourceField, config) {
 ...
 */
async function mapTransformEntriesToFields(entries, transform) {
    const fieldsObject = {};
    for (const entryRecord of entries) {
        // Extract sourceScript using NowIncludeShape for module and Now.include support
        const sourceScript = await now_include_plugin_1.NowIncludeShape.fromRecord(entryRecord, entryRecord.get('source_script'), transform);
 ...
 */
async function mapTransformScriptsToShape(scripts, transform) {
    if (scripts.length === 0) {
        return undefined;
    }
    const scriptShapes = [];
    for (const scriptRecord of scripts) {
        const script = await now_include_plugin_1.NowIncludeShape.fromRecord(scriptRecord, scriptRecord.get('script'), transform);
        scriptShapes.push(scriptRecord.transform(({ $ }) => ({
            $id: $.val(now_id_plugin_1.NowIdShape.from(scriptRecord)),
...
// PLUGIN DEFINITION
// ============================================================================
exports.ImportSetPlugin = sdk_build_core_1.Plugin.create({
    ...
    records: {
        ...
        sys_transform_map: {
            ...
            async toShape(record, { descendants, transform }) {
                ...
                const mapScript = await now_include_plugin_1.NowIncludeShape.fromRecord(record, record.get('script'), transform);
    ...
    shapes: [
        {
            ...
            async toRecord(callExpression, { factory, diagnostics, config }) {
                ...
                if (runScript && script.isDefined() && script.isUnresolved()) {
                    diagnostics.error(script, `Unable to resolve the script reference, ensure the imported module is within the ${config.serverModulesDir} directory.`);
                }
                // Validate: script should not be provided when runScript is false/undefined
                if (!runScript && script.isDefined()) {
                    diagnostics.error(script, `Property 'script' cannot be used when 'runScript' is false or undefined. Remove the 'script' property or set 'runScript: true'.`);
                }
                ...
                const transformMapRecord = await factory.createRecord({
                    ...
                    properties: arg.transform(({ $ }) => ({
                        ...
                        script: $.from('script')
                            ...
                            ?.toString((n) => `${n}({{PARAMS}})`, ['source', 'target', 'map', 'log', 'isUpdate']) ?? v)
                            .toCdata()
                ...
                if (fieldsArg.isDefined()) {
                    ...
                    for (const [targetField, fieldShape] of fieldsObject.entries()) {
                        ...
                        if (fieldShape.isString()) {
                            ...
                            entryRecords.push(await factory.createRecord({
                                source: callExpression,
                            ...
                            if (!coalesce && coalesceCaseSensitive.isDefined()) {
                                diagnostics.error(coalesceCaseSensitive, `Field '${targetField}': Property 'coalesceCaseSensitive' cannot be used when 'coalesce' is false or undefined. Remove the 'coalesceCaseSensitive' property or set 'coalesce: true'.`);
                            }
                            if (!coalesce && coalesceEmptyFields.isDefined()) {
                                diagnostics.error(coalesceEmptyFields, `Field '${targetField}': Property 'coalesceEmptyFields' cannot be used when 'coalesce' is false or undefined. Remove the 'coalesceEmptyFields' property or set 'coalesce: true'.`);
                            }
                            ...
                            if (!useSourceScript && sourceScriptValue.isDefined()) {
                                diagnostics.error(sourceScriptValue, `Field '${targetField}': Property 'sourceScript' cannot be used when 'useSourceScript' is false or undefined. Remove the 'sourceScript' property or set 'useSourceScript: true'.`);
                            }
                            // Validate: sourceScript is required when useSourceScript is true
                            if (useSourceScript && !sourceScriptValue.isDefined()) {
                                diagnostics.error(useSourceScriptValue, `Field '${targetField}': Property 'sourceScript' is required when 'useSourceScript' is true. Provide a 'sourceScript' function or set 'useSourceScript: false'.`);
                            }
                            entryRecords.push(await factory.createRecord({
                                source: callExpression,
                                table: 'sys_transform_entry',
                ...
                for (const script of scripts) {
                    ...
                    scriptRecords.push(await factory.createRecord({
                        source: callExpression,
                        ...
                        properties: obj.transform(({ $ }) => ({
                            ...
                            script: $.from('script')
                                .map((v) => v
                                .if(server_module_plugin_1.ModuleFunctionShape)
                                ?.toString((n) => `${n}({{PARAMS}})`, ['source', 'map', 'log', 'target']) ?? v)
                                .toCdata()
...
//# sourceMappingURL=import-sets-plugin.js.map