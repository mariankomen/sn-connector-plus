...
function generateListColumnProps(column, index, diagnostics) {
    const props = column
        ...
        .properties();
    (0, utils_1.generateDeprecatedDiagnostics)(column, diagnostics);
    return props;
}
function getListUpdateName(list) {
...
exports.ListPlugin = sdk_build_core_1.Plugin.create({
    name: 'ListPlugin',
    records: {
        sys_ui_list: {
            composite: true,
                    ...
                    value: new sdk_build_core_1.CallExpressionShape({
                        ...
                        args: [
                            record.transform(({ $ }) => ({
                                ...
                                view: $.map((v) => v.equals(DEFAULT_VIEW)
                                    ? new sdk_build_core_1.IdentifierShape({ source: record, name: 'default_view' })
                                    : (v.ifString() ?? v.toRecordId())),
                                parent: $.def(''),
                                relationship: $.def(''),
                                columns: $.val(descendants
                                    .query('sys_ui_list_element')
                                    .sort((a, b) => a.get('position').toNumber().getValue() -
            ...
            async toRecord(callExpression, { factory, diagnostics }) {
                ...
                const list = await factory.createRecord({
                    ...
                    properties: arg.transform(({ $ }) => ({
                        ...
                        parent: $,
                        relationship: $.map((v) => v.if([sdk_build_core_1.Record, sdk_build_core_1.StringShape])),
                        sys_domain: $.val('global'), // TODO: Domain support?
                    })),
                });
                const columnRecords = [];
                for (const [index, column] of columns.entries()) {
                    columnRecords.push(await factory.createRecord({
                        source: callExpression,
                        table: 'sys_ui_list_element',
                        properties: {
                            list_id: list,
