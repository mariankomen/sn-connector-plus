...
const dashboard_component_resolver_1 = require("./dashboard-component-resolver");
const dashboard_component_property_defaults_1 = require("./dashboard-component-property-defaults");
const transformWidget = (widget) => widget.transform(({ $ }) => ({
    $id: $.val(now_id_plugin_1.NowIdShape.from(widget)),
    component: $.map((v) => (0, dashboard_component_resolver_1.resolveComponentToName)(v.toString().getValue())),
    componentProps: $.from('component_props')
        .map((v) => {
        const json = v.ifString()?.getValue();
        const props = json ? JSON.parse(json) : v.getValue();
        return (0, dashboard_component_property_defaults_1.removeRestrictedValues)(props);
    })
        .def(dashboard_component_property_defaults_1.DEFAULT_PROPERTY_VALUES),
...
const createWidgetProperties = (widgetData, canvas) => widgetData.transform(({ $ }) => ({
    canvas: $.val(canvas),
    component: $.map((v) => (0, dashboard_component_resolver_1.resolveComponentToSysId)(v.toString().getValue())),
    component_props: $.from('componentProps').map((v) => {
        const props = v.getValue() || {};
        return JSON.stringify({ ...dashboard_component_property_defaults_1.DEFAULT_PROPERTY_VALUES, ...props });
    }),
 ...
 */
exports.DashboardPlugin = sdk_build_core_1.Plugin.create({
            ...
            toShape(record, { descendants }) {
                ...
                const tabs = descendants
                    ...
                    return tab.transform(({ $ }) => ({
                        $id: $.val(now_id_plugin_1.NowIdShape.from(tab)),
                        name: $,
                ...
                const visibilities = descendants.query('par_dashboard_visibility').map((visibility) => {
                    return visibility.transform(({ $ }) => ({
                        $id: $.val(now_id_plugin_1.NowIdShape.from(visibility)),
                        experience: $.from('experience').def(''),
                    }));
                });
                const permissions = descendants.query('par_dashboard_permission').map((permission) => permission.transform(({ $ }) => {
                    ...
                    const permissionSubject = hasUser
                        ...
                        : hasRole
                            ...
                            : hasGroup
                                ...
                                : {};
                    return {
                        $id: $.val(now_id_plugin_1.NowIdShape.from(permission)),
    ...
    shapes: [
        {
            ...
            async toRecord(source, { factory }) {
                ...
                if (visibilities.length === 0) {
                    ...
                    visibilityRecords.push(await factory.createRecord({
                        source: source,
                ...
                for (const permission of permissions) {
                    ...
                    permissionRecords.push(await factory.createRecord({
                        source: permission,
...
//# sourceMappingURL=dashboard-plugin.js.map