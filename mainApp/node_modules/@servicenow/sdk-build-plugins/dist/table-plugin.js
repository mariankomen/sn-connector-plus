...
const ColumnSchema = zod_1.z
    ...
    '@_read_only_option': zod_1.z.string().optional(),
            ...
            async toRecord(callExpression, { config, factory, transform, diagnostics, compiler }) {
                            ...
                            properties: {
                                ...
                                index_col_name: colNames
                                    ...
                                    if (!columnIdsMap.has(colName)) {
                                        diagnostics.error(index, `Column '${colName}' not found in table '${tableName.getValue()}'`);
                                        return '';
                                    }
                ...
                if (table.get('label').isArray()) {
                    ...
                    for (const doc of documentation) {
                        ...
                        if (language) {
                            if (!column_plugin_1.BCP47_LANGUAGE_TAG_PATTERN.test(language)) {
                                diagnostics.error(doc.getOriginalNode(), `'language' must be a valid BCP 47 language tag (e.g., 'en', 'es', 'en-US', 'zh-Hans')`);
                            }
...
function tableDefToRecordProperties(tableDef, tableDefaultLanguage = 'en') {
    const sysDbObject = {
        name: tableDef.name,
