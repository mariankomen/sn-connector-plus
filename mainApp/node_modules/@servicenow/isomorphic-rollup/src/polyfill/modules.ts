...
type PolyfillResolvedType = "no-op" | "polyfill" | "no-resolve";

export type PolyfillFilter = Record<Polyfill, PolyfillResolvedType>;

const EMPTY_PATH = POLYFILLS["empty.js"];

export function getModules(polyfillFilter?: Partial<PolyfillFilter>) {
  const libs = new Map<Polyfill, string | undefined>();
  ...
  if (polyfillFilter) {
    for (const key of libs.keys()) {
      const polyfillResolveType = polyfillFilter[key];

      if (polyfillResolveType === "no-op") {
        libs.set(key, EMPTY_PATH);
      } else if (polyfillResolveType === "no-resolve") {
        libs.delete(key);
      } else if (polyfillResolveType === "polyfill") {
        continue;
      }
    }
  }
