(function process(request, response) {
    ...
    try {
        ...
        if (httpStatus >= 200 && httpStatus < 300) {
            ...
            var recordTypes = [];
            if (describeData.recordTypeInfos && Array.isArray(describeData.recordTypeInfos)) {
                for (var i = 0; i < describeData.recordTypeInfos.length; i++) {
                    var recordType = describeData.recordTypeInfos[i];
                    
                    recordTypes.push({
                        name: recordType.name,
                        recordTypeId: recordType.master ? 'Master' : recordType.recordTypeId,
                        master: recordType.master || false,
                        defaultRecordTypeMapping: recordType.defaultRecordTypeMapping || false,
                        active: recordType.active !== false
                    });
                }
            }
        ...
        gs.error('Error in Salesforce record types handler: ' + err.message + '\nStack: ' + err.stack);
        response.setStatus(500);
        writer.writeString(JSON.stringify({
            success: false,
